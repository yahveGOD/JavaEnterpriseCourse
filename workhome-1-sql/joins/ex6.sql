...
select mems.firstname || ' ' || mems.surname as member, 
	...
	case 
		when mems.member_id = 0 then
			bks.slots*fac.guest_cost
		else
			bks.slots*fac.member_cost
	end as cost
        from
                members mems                
                inner join bookings bks
                        on mems.member_id = bks.member_id
                inner join facilities fac
                        on bks.facility_id = fac.facility_id
        where
		bks.start_time >= '2012-09-14' and 
		bks.start_time < '2012-09-15' and (
			(mems.member_id = 0 and bks.slots*fac.guest_cost > 30) or
			(mems.member_id != 0 and bks.slots*fac.member_cost > 30)
		)
order by cost desc;            