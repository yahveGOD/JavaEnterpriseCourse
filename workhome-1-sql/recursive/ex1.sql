...
with recursive recmembers(rec_member) as(
	select recommended_by from members where member_id = 27
	union all
	select mems.recommended_by
		from rec_members rec
		inner join members mems
			on mems.member_id = rec.rec_member  
  )
select rec.rec_member,mems.firstname,mems.surname
from rec_members rec
	inner join members mems
		on rec.rec_member = mems.member_id
