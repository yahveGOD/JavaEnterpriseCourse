...
with recursive recmembers(member_id) as(
  	select member_id from members where recommended_by =1
  	union all
  	select mems.member_id
  		from recmembers rec
  		inner join members mems
  			on rec.member_id = mems.recommended_by
  )
  select rec.member_id,mems.firstname,mems.surname
  from recmembers rec
  inner join members mems
  	on rec.member_id = mems.member_id
order by member_id