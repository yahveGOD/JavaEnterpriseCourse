...
select 	facs.name as name,
	facs.initial_outlay/((sum(case
			when member_id = 0 then slots * facs.guest_cost
			else slots * member_cost
		end)/3) - facs.monthly_maintenance) as months
	from bookings bks
	inner join facilities facs
		on bks.facility_id = facs.facility_id
	group by facs.facility_id
order by name;   