...
@RestController
@RequestMapping("api/v1/game_mode")
...
public class GameModeController {
    ...
    {
        String json = jsonMapper.convertToJsonString(gameModeService.findById(id));
        return json;
    }

    @PutMapping("/edit/{id}")
    public void editUpdate(@PathVariable("id") Long id, GameModeDto gameModeDto) {
        gameModeService.update(id, gameModeDto);
    }

    @GetMapping("/all")
    public String findAll() {
        String json = jsonMapper.convertToJsonString(gameModeService.findAll()
                .stream().map(GameModeDtoMapper::convertDtoToEntity)
                .toList());
        return json;
    }
    ...
    public ResponseEntity<String> getJson() {
        try {
            String json = serializeToJson(gameModeService.findAll()
                    .stream().map(GameModeDtoMapper::convertDtoToEntity)
                    .toList());
